...
int respondToClientInOneLine(const char* response, const HttpClient* pHttpClient){
    if(send(pHttpClient->fd, response, std::strlen(response), 0)==-1){
        ...
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": Responding "<<std::string(response); 
        pthread_mutex_unlock(&lock);
        return 0;
    }
}
...
int respondToClientInManyLines(const std::vector<char>& msg, const int size, const HttpClient* pHttpClient, const Response& response){
    ...
    pthread_mutex_lock(&lock);
    logmsg <<pHttpClient->id<<": Responding "<<response.getResponseLine(); 
    pthread_mutex_unlock(&lock);
    return 0;
}
...
int relayChunkedMsgToServer(std::vector<char>& msg, int& size, const HttpClient* pHttpClient, const Client* pProxyClient){
    ...
    if(msg[size-5]=='0'&&msg[size-4]=='\r'&&msg[size-3]=='\n'&&msg[size-2]=='\r'&&msg[size-1]=='\n'){//send all in the first msg
        if(send(pHttpClient->fd, &(msg.data()[0]), size, 0)==-1){
                pthread_mutex_lock(&lock);
    ...
    if(msg[0]=='0'|| size==0){// modified, last chunk
        //test
...
int relayChunkedMsgToClient(std::vector<char>& buf, int& recvSize, const HttpClient* pHttpClient, const Client* pProxyClient){
    ...
    for(int i=0;i<recvSize;++i)
        std::cout<<buf[i];
    std::cout<<std::endl;
    if(buf[recvSize-5]=='0'&&buf[recvSize-4]=='\r'&&buf[recvSize-3]=='\n'&&buf[recvSize-2]=='\r'&&buf[recvSize-1]=='\n'){//send all in the first msg
    ...
    while(true){
        ...
        std::cout<<"after sending, send size is: "<<recvSize<<std::endl;
        if(buf[0]=='0'|| recvSize==0){//modified
        //test
...
int revalidateWithTagAndSendBack(LRUCache& lruCache, const bool isETag, const std::string& uri, const std::vector<char>& clientMsg, const int clientSize, const Request& clientReq, const Response& cacheResp, const HttpClient* pHttpClient, const Client* pProxyClient){
    ...
    pthread_mutex_lock(&lock);
    logmsg<<pHttpClient->id<<": Requesting \""<<clientReq.getReqLine()<<"\" from "<<clientReq.getTargetHost()<<std::endl<<std::endl; 
    pthread_mutex_unlock(&lock);
    ...
    pthread_mutex_lock(&lock);
    logmsg<<pHttpClient->id<<": Received \""<<recvResp.getResponseLine()<<"\" from "<<clientReq.getTargetHost()<<std::endl<<std::endl; 
    pthread_mutex_unlock(&lock);
    //test
    ...
    if(recvResp.getStatus()==304){// do we modify the Date inside the cache? It depends!!!!
        ...
        outputFile4.close();
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": Responding "<<lruCache.getResponse(uri).getResponseLine(); 
        pthread_mutex_unlock(&lock);
        return 0;
    }
        ...
        if(recvResp.getIsChunked()){//directly send to client
            ...
            pthread_mutex_lock(&lock);
            logmsg<<pHttpClient->id<<": not cacheable because it's chunked"<<std::endl<<std::endl; 
            pthread_mutex_unlock(&lock);
            if(relayChunkedMsgToClient(recvMsg, recvSize, pHttpClient, pProxyClient)==-1)
                return -1;
            pthread_mutex_lock(&lock);
            logmsg <<pHttpClient->id<<": Responding "<<recvResp.getResponseLine(); 
            pthread_mutex_unlock(&lock);
            return 0;
        }
...
int handleGETresponse(const HttpClient* pHttpClient, LRUCache& lruCache, const std::string& uri, const Response& recvResp, const std::vector<char>& recvMsg, const int recvSize){
    ...
    if(!shouldNotCache(recvResp)){
        ...
        if(recvResp.getMaxAge()!=""){
            ...
            pthread_mutex_lock(&lock);
            logmsg <<pHttpClient->id<<": cached, but requires re-validation"<<std::endl<<std::endl; 
            pthread_mutex_unlock(&lock);
        }
        //test
        const Response& temResp=lruCache.getResponse(uri);
        ...
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": not cacheable because response status is not 200 OK"<<std::endl<<std::endl; 
        pthread_mutex_unlock(&lock);
    }
    else if(shouldNotCache(recvResp)==2){
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": not cacheable because there's no-store or private tag"<<std::endl<<std::endl; 
        pthread_mutex_unlock(&lock);
    }
    else if(shouldNotCache(recvResp)==3){
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": not cacheable because there's no-cache tag but without revalidation header"<<std::endl<<std::endl; 
        pthread_mutex_unlock(&lock);
    }
    else if(shouldNotCache(recvResp)==4){
        pthread_mutex_lock(&lock);
        logmsg <<pHttpClient->id<<": not cacheable because there's no expiration and revalidation header"<<std::endl<<std::endl; 
        pthread_mutex_unlock(&lock);
    }
...
int validateFresh(Response& response, const HttpClient* pHttpClient)
{
    ...
    if(response.getMaxAge() != "")
    {
        ...
        {   
            pthread_mutex_lock(&lock);
            logmsg << pHttpClient->id << ": in cache, valid" <<std::endl<<std::endl;
            pthread_mutex_unlock(&lock);
            //it is fresh
            return 0;
        }
    ...
    pthread_mutex_lock(&lock);
    logmsg << pHttpClient->id << ": in cache, requires validation" << std::endl<< std::endl;
    pthread_mutex_unlock(&lock);
    return -2;
}
